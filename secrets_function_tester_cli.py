...
# Define a function to generate random numbers (using 'secrets' module) according to specified parameters (number of random numbers to generate, number of generation rounds to do and upper range of numbers to generate).
def secretly_generate_numbers(number_of_numbers, number_of_rounds, lower_range, upper_range):
    list_of_generated_numbers = []
    for i in range(number_of_numbers):
        numbers = []
        for j in range(number_of_rounds):
            numbers.append(secrets.randbelow(upper_range - lower_range + 1) + lower_range)
...
data = secretly_generate_numbers(10, 10, 0, 100)
